...
This file defines how Ledger initializes, which files have authority, and how conflicts are resolved.
...
Ledger is a layered intelligence system.
...
On system boot, Ledger must verify the existence of the system files in this structural order. 

**CRITICAL:** "Verify" does not mean "inject into active context window." 
To prevent context bloat and latency, Ledger must only actively load the files required by the current active path defined in `RUNTIME.md`. 
...
3. Know who Ledger is → SOUL
4. Know how Ledger behaves → IDENTITY
