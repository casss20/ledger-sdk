...
async def test_racing_capability_consumption_basic(kernel, db):
...
async def test_capability_exhausted_stays_exhausted(kernel, db):
    ...
    await db.execute(
        """
        INSERT INTO capabilities (token_id, actor_id, action_scope, resource_scope, expires_at, max_uses, uses)
        VALUES ($1, $2, 'test.*', 'test', NOW() + interval '1 hour', 2, 2)
        """,
        token_id, actor_id
    )
...
async def test_consumption_before_execution_semantic(kernel, db):
    ...
    await db.execute(
        """
        INSERT INTO capabilities (token_id, actor_id, action_scope, resource_scope, expires_at, max_uses)
        VALUES ($1, $2, 'fail.*', 'test', NOW() + interval '1 hour', 1)
        """,
        token_id, actor_id
    )

    # We can't easily make the executor fail from the test, so we verify the semantic
    # by checking that a successful execution does consume a use. The key invariant is:
    # consumption happens during validation (before execution), so even if the executor
    # threw, the use would already be spent.
    action = Action(
        action_id=uuid.uuid4(),
        actor_id=actor_id,
        actor_type='agent',
    ...
    # The action may or may not execute successfully depending on executor behavior,
    # but the capability should be consumed regardless.
...
async def test_racing_capability_stress(kernel, db):
