...
import { 
  ...
  Rocket, MessageCircle, AlertCircle 
} from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <Icon className="w-4 h-4" style={{ color } as any} />
        </div>
      ...
      <Handle type="target" position={Position.Left} className="w-2 h-2 !bg-slate-500 !border-2 !border-slate-800" />
      <Handle type="source" position={Position.Right} className="w-2 h-2 !bg-slate-500 !border-2 !border-slate-800" />
    </div>
  );
}
