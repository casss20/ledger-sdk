...
Governance tokens (`gt_`) are the atomic unit of accountability in Ledger. Every time an agent attempts an action, Ledger creates a token — an immutable, cryptographically signed record of the governance decision.

Think of them like Stripe's `pm_` PaymentMethod tokens: they reference a decision stored in Ledger's vault, are non-portable to other systems, and accumulate over time to create switching costs.
...
Agent requests action
    ↓
Ledger evaluates against policies
    ↓
...
**This accumulation creates data gravity.** Like Stripe's card tokens make migration painful (customers must re-enter cards), Ledger's governance tokens make migration legally hazardous — your entire audit history lives in Ledger's vault.
...
```python
record = ledger.audit.get("gt_1Aa2Bb3Cc4Dd5Ee6Ff7Gg8Hh")
```

### By agent

```python
records = ledger.audit.query(agent_id="email-agent-01", limit=100)
```
...
```python
records = ledger.audit.query(policy_name="refund-approval-over-1000")
```
...
is_tampered = ledger.audit.verify_hash(record)
...
```python
ledger.config.set_retention_policy({
    hot_days=90,
    warm_years=7,
