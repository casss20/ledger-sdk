...
Citadel keeps trust scoring small in the active wedge path. Trust is decision evidence and policy context; it is not a separate authorization engine.
...
### Active Trust Factors

| Factor | Weight | Description |
| --- | ---: | --- |
| `identity_verification` | 0.30 | Cryptographic identity verification status. |
| `operational_health` | 0.35 | Health score, quarantine state, and suspicious action volume. |
| `governance_record` | 0.35 | Recent violations and budget pressure. |

### Compatibility Bands

The runtime still accepts historical band values for stored data and API compatibility. The wedge-focused operating model is REVOKED / STANDARD / TRUSTED; PROBATION and HIGHLY_TRUSTED are compatibility states until callers can migrate safely.

Trust can only add constraints or context. It cannot bypass the kill switch, remove a policy denial, override spend enforcement, or issue execution authority by itself.

### Trust Audit Events

Every trust-aware decision stores `trust_snapshot_id` for deterministic replay. Trust score and factor breakdowns are recorded as decision evidence and surfaced in the dashboard from stored snapshots.

### Resources

- [Trust Scoring](docs/public/core-concepts/trust-scoring.md) - Active score computation and factors
---
...
**Trust Architecture:**
- [x] Three-factor operational trust model
- [x] Append-only trust snapshots with full audit replay
- [x] Policy integration as trust-aware context
- [x] Kill switch and spend enforcement remain authoritative
