# Ledger Core Hardening Pass Log
...
- **Red test commit:** `195ce28` — `test: reproduce policy resolver dict-condition crash (failing)`
- **Fix commit(s):** `9c3698a` — `fix: handle dict conditions in policy resolver + honor PENDING_APPROVAL effect (Issue 1)`
- **Files changed:**
  - `src/ledger/policy_resolver.py`
  - `src/ledger/approval_service.py`
- **Tests added:**
...
- **Category:** Policy resolver — schema deserialization + approval handoff
...
- **Red test commit:** `9d8990e` — `test: reproduce capability race and consumption semantic bugs (failing)`
- **Fix commit(s):**
  - `026f146` — `fix: capability scope matching for .* wildcard patterns`
  - `2a966df` — `fix: atomic capability consumption via PostgreSQL FOR UPDATE function (Issue 2)`
- **Files changed:**
  - `src/ledger/precedence.py`
- **Tests added:**
...
- **Category:** Capability — check-then-act without atomic compare-and-swap
...
- **Red test commit:** `4b86ef3` — `test: reproduce idempotency race — 10 concurrent requests insert duplicate actions (failing)`
- **Fix commit:** `f2fcd97` — `fix: atomic idempotent action insert with ON CONFLICT + retry lookup (Issue 2.5)`
- **Files changed:**
  - `src/ledger/repository.py`
  - `src/ledger/execution/kernel.py`
- **Tests added:**
...
- **Category:** Action ingestion — check-then-act without atomic insert
...
- **Red test commit:** `a99643a` — `test: reproduce audit chain hash integrity break under 50 concurrent actions (failing)`
- **Fix commit:** `a097ffe` — `fix: serialize audit chain appends with PostgreSQL advisory lock (Issue 3)`
- **Files changed:**
  - `src/ledger/repository.py`
- **Tests added:**
...
- **Category:** Audit — shared mutable state (prev_hash) without lock
