...
CITADEL_DIR = Path(__file__).resolve().parent.parent.parent / "citadel"
...
def core_governance(path: Path_) -> str:
    ...
    for name in PATH_FILES[path]:
        body = _read(CITADEL_DIR / "core" / name)
        if body:
            parts.append(f"## [{name}]\n{body}")
    return "\n\n---\n\n".join(parts)
...
def agent_identity(agent: str) -> str:
    folder = CITADEL_DIR / "agents" / agent.lower()
    parts = []
...
def session_context(session_id: str) -> str:
    folder = CITADEL_DIR / "context" / session_id
    parts = []
